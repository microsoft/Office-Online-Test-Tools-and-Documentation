...
Client Changes: Invoking your App on Android
	...
	* UserId: It will be an optional param and will be set whenever we will have it. Third party should use to verify that the sign-is requested for the User signed-in to their App.
	 
...
..  _App2AppSigninIntent.java: https://github.com/Microsoft/Office-Online-Test-Tools-and-Documentation/blob/master/samples/android/App2AppSigninIntent.java
	...
	* ResonponseUrlQueryParams: Again this is exactly same as what we are getting in iOS minus action param, following are values of it in success and failure
		code=abcdefg&tk=http://contoso.com&sc=xyz [during RESULT_OK]
		error=invalied_request&error_decstption=”optional human readable message” [during RESULT_CANCELLED or anything else]
