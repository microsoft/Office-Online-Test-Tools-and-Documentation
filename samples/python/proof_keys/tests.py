...
from proof_keys import ProofKeyDiscoveryData, ProofKeyValidationInput, ProofKeyHelper


__author__ = 'Tyler Butler <tyler.butler@microsoft.com>'

ProofKeyTestCase = namedtuple('ProofKeyTestCase', ('expected_result', 'input_data'))

discovery = ProofKeyDiscoveryData(
    value="",
    ...
    exponent="AQAB",
    oldvalue="",
...
helper = ProofKeyHelper(discovery)


class ProofKeyTests(unittest.TestCase):
    def test_proof_current_key1(self):
        """Verify X-WOPI-Proof with current discovery proof key."""

        test_data = ProofKeyValidationInput(
        ...
        self.assertTrue(helper.validate(test_data))

    def test_proof_current_key2(self):
        """Verify X-WOPI-Proof with current discovery proof key."""
    ...
    def test_old_proof_current_key1(self):
        """Verify X-WOPI-ProofOld with current discovery proof key."""
    ...
    def test_old_proof_current_key2(self):
        """Verify X-WOPI-ProofOld with current discovery proof key."""
    ...
    def test_proof_old_key1(self):
        """Verify X-WOPI-Proof with old discovery proof key."""
    ...
    def test_invalid1(self):
        ...
        self.assertFalse(helper.validate(test_data))
